...
// SERVERLESS FIX: Keep pools minimal to avoid choking your Supabase limits
let pool;
if (!pool) {
  pool = new Pool({ 
    connectionString: process.env.DATABASE_URL,
    max: 1, // Single connection per warm serverless invocation
    idleTimeoutMillis: 30000,
    connectionTimeoutMillis: 2000,
  });
}

// Daraja base URL — sandbox by default. Set MPESA_ENV=production
const DARAJA_BASE =
...
// PREMIUM PLAN PRICING — The immutable source of truth
...
app.post("/api/mpesa/callback", async (req, res) => {
  try {
    ...
    // BUG FIX: Explicitly convert the object payload to a JSON string string for pg driver stability
...
// ENVIRONMENT STABILITY FIX: Prevent routing reference errors during edge deployments
try {
  ...
  // Gracefully skip execution if fileURLToPath is unavailable in production build triggers
}
